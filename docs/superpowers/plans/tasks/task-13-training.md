...
**Подписка на отдельные поля хранилища вместо `useTraining()` целиком.** Исходный вариант получал весь стор одним вызовом и передавал объект `store` в зависимости эффектов. Поскольку Zustand возвращает новый объект при каждом изменении, эффекты перезапускались непрерывно и компонент не доходил до отрисовки. Теперь каждое поле берётся отдельным селектором.

**Инициализация через `useRef` для `items`.** Массив `items` создаётся заново при каждом рендере серверного компонента, поэтому в зависимостях `useEffect` он вызывал бесконечный цикл `init`. Значение хранится в ref, а эффект зависит только от `sessionId`.

**Флаг `started` перед показом экрана завершения.** Без него компонент до первого `init` видел пустую очередь и сразу показывал «Сессия завершена».

**Улучшен подбор отвлекающих вариантов.** Исходный порядок (тот же ряд → та же азбука) для ёон выдавал знаки годзюон: きゃ получала варианты `ka, ki, ku`. Теперь приоритет: тот же ряд и тот же тип → тот же тип → тот же ряд → та же азбука. В результате きゃ различается с `kyu, kyo`, が — с `gi, gu, ge`, じ — с `za, zu, ze`, что делает выбор настоящей проверкой узнавания.

**Обработка ошибок сети в `flush`.** Вызов `syncAnswers` обёрнут в try/catch: при отсутствии связи ответы остаются в localStorage и досылаются при следующем запуске.
