...
**Экран настроек показывает лимит новых знаков в день.** Значение хранится в `user_progress.new_cards_per_day` и определяет темп прохождения; показать его пользователю дешевле, чем объяснять, почему очередь заканчивается.

**Навигация прокручивается по горизонтали.** Семь пунктов в одну строку на экране 320 пикселей дают по 46 пикселей на пункт — «Грамматика», «Избранное», «Прогресс» и «Сегодня» не помещаются. Добавлены `overflow-x-auto` и `whitespace-nowrap`: на узких экранах список прокручивается, на широких растягивается.

**Атрибуция KanjiVG продублирована на экране настроек.** На карточке знака она есть, но лицензия CC BY-SA 3.0 требует указания при распространении работы в целом, а не только на странице использования.
